...
mens_stores.each do |store|
  puts "Stores with mens apparel only with names #{store.name} and revenues #{store.annual_revenue}"
end

womens_stores = Store.where('annual_revenue > ?', 1000000).where(womens_apparel: "true")

womens_stores.each do |store|
  puts "Stores with womens apparel only with names #{store.name} and revenues #{store.annual_revenue}"
end
